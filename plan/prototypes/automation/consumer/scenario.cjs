...
module.exports = async function run({ lib, common, core, rxjs, label }) {
  ...
  for (const [name, value] of Object.entries({ HttpModule, HttpService, AxiosError, isAxiosError }))
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (url.pathname === '/redirect') { res.writeHead(302, { Location: '/echo?redirected=1' }); return res.end(); }
      if (url.pathname === '/login') { res.writeHead(200, { 'Set-Cookie': 'sid=abc; Path=/' }); return res.end('ok'); }
      if (url.pathname.startsWith('/status/')) { res.writeHead(Number(url.pathname.slice(8)), { 'Content-Type': 'application/json' }); return res.end('{"err":true}'); }
  ...
  const TOKEN = 'TOKEN';
  class TokenService { get() { return 'di-token'; } }
  Injectable()(TokenService);
  class TokenModule {}
  Module({ providers: [TokenService, { provide: TOKEN, useValue: 'cfg' }], exports: [TokenService, TOKEN] })(TokenModule);
  class AuthInterceptor {
    constructor(tokens) { this.tokens = tokens; }
    intercept(request, next) {
      request.options.headers = { ...request.options.headers, authorization: `Bearer ${this.tokens.get()}` };
  ...
  class SyncHttp {}
  Module({ imports: [HttpModule.register({ baseURL: base, headers: { 'X-Module': 'm' }, maxRedirects: 5, withCredentials: true, interceptors: [fnInterceptor] })], exports: [HttpModule] })(SyncHttp);
  class AsyncHttp {}
  Module({
    imports: [HttpModule.registerAsync({ imports: [TokenModule], inject: [TOKEN], useFactory: cfg => ({ baseURL: `${base}/async-${cfg}`, interceptors: [AuthInterceptor] }) })],
    exports: [HttpModule],
  })(AsyncHttp);

  // Two consumers, each injecting HttpService from its own module
  class SyncClient { constructor(http) { this.http = http; } }
  Inject(HttpService)(SyncClient, undefined, 0); Injectable()(SyncClient);
  class AsyncClient { constructor(http) { this.http = http; } }
  Inject(HttpService)(AsyncClient, undefined, 0); Injectable()(AsyncClient);
  class SyncFeature {} Module({ imports: [SyncHttp], providers: [SyncClient], exports: [SyncClient] })(SyncFeature);
  class AsyncFeature {} Module({ imports: [AsyncHttp], providers: [AsyncClient], exports: [AsyncClient] })(AsyncFeature);
  class AppModule {} Module({ imports: [SyncFeature, AsyncFeature] })(AppModule);
  ...
  try {
    ...
    const id = sync.axiosRef.interceptors.request.use(c => { c.headers['X-Axios-Ref'] = 'yes'; return c; });
    ...
    // errors
    const error = await firstValueFrom(sync.get('/status/404')).then(() => null, e => e);
    ...
    const refused = await firstValueFrom(sync.get('http://127.0.0.1:1/')).then(() => null, e => e);
    ...
    assert.ok(sync.get('/echo') instanceof Observable, 'returns the consumer rxjs Observable');
  } finally {
    await app.close();
