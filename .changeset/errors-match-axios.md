...
- `maxContentLength` exceeded now rejects with `ERR_BAD_RESPONSE` (was `ERR_FR_MAX_CONTENT_LENGTH_EXCEEDED`), enforced while the response streams in rather than after buffering the whole body (for an uncompressed response - a compressed one is still buffered before the decoded size is checked).
