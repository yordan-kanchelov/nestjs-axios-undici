...
- 🚀 **About 2x the throughput, half the latency.** In our benchmark (a NestJS endpoint fanning out 5 upstream calls under load, with the same logging interceptor on both sides), it served **1.8-2.2x the requests per second** of `@nestjs/axios`, with **46-55% lower average and 50-58% lower p95 latency**, on Node.js 20, 22, 24 and 26. The benchmarks run in CI on every change; see the [results](docs/benchmarks.md).
...
- ✅ **Tested on Node.js 20, 22, 24 and 26**, with a performance regression check on every pull request.
