...
- 2026-09-25: PR open (`claude/perf-request-path`): `perf: cache merged headers and the interceptor chain`. Defaults + module headers are pre-merged per method and cached per `defaults` object (invalidated by a version counter bumped on any write to `axiosRef.defaults.headers` or a method bucket); the interceptor handler chain is cached and rebuilt only when interceptors are added or replaced; `executeRequest` registers one `.then(onFulfilled, onRejected)` instead of a 3-link chain (a synchronous throw from `request()`, such as an invalid URL, still propagates raw, as in @nestjs/axios). Skipped: skipping re-normalization per subscription (the benchmarks never re-subscribe, and a cached body would break `retry()` with one-shot bodies), and removing the AxiosHeaders Proxy (interceptors is already well inside the threshold). Local CPU/req vs claude/v1.0.0: get -7.8%, post -18.0%, config +2.8%, error +2.4%, interceptors -14.1%. vs 0.6.1: get +9.7% (target ≤ +10% met), post +19.8% (default headers, Content-Type default and abort signal are new work), config +5.0%, error +4.3%. New `http.service.perf-cache.spec.ts` covers cache invalidation. 537/537 Jest tests pass on Node 24.
