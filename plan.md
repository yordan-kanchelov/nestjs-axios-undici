...
- [~] **C. test: API-surface parity and type-level drop-in checks** (`claude/api-parity-checks`, PR open)
  ...
  - `tests/compat/api-surface.spec.ts` (Jest, runs in `test:jest`): fails on any un-allowlisted missing member, and on any allowlisted member that starts to exist (stale allowlist). Allowlisted gaps, all tracked to phase 2 items above:
    ...
    - `axiosRef`: not callable, and missing `getUri`, `create`, `postForm`, `putForm`, `patchForm`, `query` (needs "make axiosRef a real axios instance").
    - `axiosRef.defaults`: `transitional`, `adapter`, `transformRequest`, `transformResponse`, `timeout`, `xsrfCookieName`, `xsrfHeaderName`, `maxContentLength`, `maxBodyLength`, `env`, `validateStatus` not populated (same item).
