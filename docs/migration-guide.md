...
- **Known limitation, not a behaviour change we made:** a slow `responseType: 'stream'` consumer can occasionally see `UND_ERR_SOCKET`/"other side closed" against a server with a short `keepAliveTimeout` (Node's own default is 5s), even though the full body was actually delivered - an undici transport bug, not reproduced with axios' own Node `http` transport. For every other `responseType` (`json`/`text`/`arraybuffer`/`blob`, already fully buffered in memory regardless), setting `maxRate`/`onDownloadProgress` on a response whose `Content-Length` is within `maxContentLength` works around it (verified directly against axios' own conformance test for exactly this scenario - see [`maxRate`](/docs/axios-supported-options.md#progress-callbacks-maxrate-and-formserializer)). `responseType: 'stream'` is deliberately **not** covered by that same workaround, even with `maxRate`/`onDownloadProgress` set - buffering an unbounded streamed download ahead of time to avoid the race would defeat the point of streaming - so a slow `'stream'` consumer, with or without those set, and a chunked/unknown-length response either way, remain exposed. See [`plan/reports/undici-slow-consumer.md`](/plan/reports/undici-slow-consumer.md).
