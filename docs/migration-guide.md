...
- **Known limitation, not a behaviour change we made:** a slow `responseType: 'stream'` consumer with neither `maxRate` nor `onDownloadProgress` set can occasionally see `UND_ERR_SOCKET`/"other side closed" against a server with a short `keepAliveTimeout` (Node's own default is 5s), even though the full body was actually delivered - an undici transport bug, not reproduced with axios' own Node `http` transport. Setting `maxRate`/`onDownloadProgress` on a response with a known `Content-Length` works around it (verified directly against axios' own conformance test for exactly this scenario - see [`maxRate`](/docs/axios-supported-options.md#progress-callbacks-maxrate-and-formserializer)); a plain slow stream consumer, or a chunked/unknown-length response either way, does not. See [`plan/reports/undici-slow-consumer.md`](/plan/reports/undici-slow-consumer.md).
