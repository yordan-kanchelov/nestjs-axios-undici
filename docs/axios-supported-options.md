...
| `responseType: 'json' \| 'text' \| 'arraybuffer' \| 'blob' \| 'stream'` | ✅ | `arraybuffer` gives a `Buffer`; `blob` gives a UTF-8 string, matching axios in Node.js (no native `Blob` decoding there); `stream` gives the undici body (a Node.js `Readable`, transparently decompressed like axios), with `maxContentLength` enforced on it as bytes are read - see below. **Known limitation** for a plain, un-metered `'stream'` consumer that reads slower than the server sends (a slow downstream sink, with neither `maxRate` nor `onDownloadProgress` set below): against a server with a short `keepAliveTimeout` (Node's own default is 5s), the stream can fail with `UND_ERR_SOCKET`/"other side closed" even though every `Content-Length` byte was actually delivered - an undici bug (not reproduced with axios' own Node `http` transport). Setting `maxRate`/`onDownloadProgress` (below) on a response with a known `Content-Length` works around it (verified against axios' own conformance test for exactly this: a 1MB download rate-limited against a 1s-`keepAliveTimeout` server). A response with no `Content-Length` (chunked, unknown length) isn't covered either way. See [`plan/reports/undici-slow-consumer.md`](/plan/reports/undici-slow-consumer.md) for the root cause, and use a longer `server.keepAliveTimeout` on servers you control, or drain the stream faster, as a workaround for the uncovered cases. |
...
| `maxRate` (a number, or `[upload, download]`) | ✅ | Throttles actual throughput (not just the progress-event rate) in both directions, via the same windowed-chunk-splitting algorithm axios' `AxiosTransformStream` uses. Throttling download this way is exactly what can trip the undici bug described above for `responseType: 'stream'` - for a response with a known `Content-Length`, this is worked around: the raw response is drained as fast as undici delivers it (bounded by `Content-Length`, so never more memory than a buffered response already uses) and only the *paced* output stays throttled, so undici's own socket handling is never left in the state that triggers the bug; a fully-delivered body that still (rarely) races and errors with `UND_ERR_SOCKET`/"other side closed" is also turned back into a normal end as a backstop. A response with no `Content-Length` (chunked, unknown length) has no safe bound to drain ahead of time and isn't covered - see [`plan/reports/undici-slow-consumer.md`](/plan/reports/undici-slow-consumer.md). |
