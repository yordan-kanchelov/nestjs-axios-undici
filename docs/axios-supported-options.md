...
| `maxContentLength` | ✅ | `ERR_BAD_RESPONSE`, "maxContentLength size of N exceeded" - same code/message as axios. Enforced against the *decompressed* size, streamed as bytes arrive - for a compressed body, the decompression itself is streamed and checked chunk by chunk (both the compressed and decompression streams are destroyed the moment the limit is crossed), so a small, highly compressible body ("gzip bomb") can't fully decompress in memory before being rejected. A per-request value wins over a module-level one. |
