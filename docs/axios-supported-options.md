...
| A `Content-Length` header that doesn't match the body's actual byte length | ⚠️ | Rejected (`ERR_BAD_REQUEST`, wrapping undici's `RequestContentLengthMismatchError` - a request-smuggling protection). axios' own Node `http` transport trusts a caller-supplied `Content-Length` and sends it as given. Leave `Content-Length` unset (the default) to have it computed correctly. |
