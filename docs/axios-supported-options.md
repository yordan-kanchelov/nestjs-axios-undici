...
| `responseType: 'json' \| 'text' \| 'arraybuffer' \| 'blob' \| 'stream'` | ✅ | `arraybuffer` gives a `Buffer`; `blob` gives a UTF-8 string, matching axios in Node.js (no native `Blob` decoding there); `stream` gives the undici body (a Node.js `Readable`, transparently decompressed like axios), with `maxContentLength` enforced on it as bytes are read - see below. **Known limitation** for a `'stream'` consumer that reads slower than the server sends (a slow downstream sink, or one throttled with `maxRate`/`onDownloadProgress` below): against a server with a short `keepAliveTimeout` (Node's own default is 5s), the stream can fail with `UND_ERR_SOCKET`/"other side closed" even though every `Content-Length` byte was actually delivered - an undici bug (not reproduced with axios' own Node `http` transport). This is **not** worked around for `'stream'` - `maxRate`/`onDownloadProgress` do not fix it there, unlike for every other `responseType` (see below), because doing so would mean either buffering an unbounded amount of a potentially huge streamed download in memory ahead of time, or shipping a workaround for undici's own socket-framing internals - both out of scope for this opt-in feature. See [`plan/reports/undici-slow-consumer.md`](/plan/reports/undici-slow-consumer.md) for the root cause, and use a longer `server.keepAliveTimeout` on servers you control, or drain the stream faster, as a workaround. |
...
| `maxRate` (a number, or `[upload, download]`) | ✅ | Throttles actual throughput (not just the progress-event rate) in both directions, via the same windowed-chunk-splitting algorithm axios' `AxiosTransformStream` uses. Throttling download this way is exactly what can trip the undici bug described above - for every `responseType` **except** `'stream'` (`json`/`text`/`arraybuffer`/`blob`, which get fully buffered in memory regardless of `maxRate`), and only when the response's own `Content-Length` is within `maxContentLength` (otherwise the streamed `maxContentLength` check needs to see the real, backpressured pace to reject an oversized body promptly), this is worked around: the raw response is drained as fast as undici delivers it - bounded by `Content-Length`, so no more memory than that buffered response already uses regardless of `maxRate` - and only the *paced* output stays throttled, so undici's own socket handling is never left in the state that triggers the bug. A fully-delivered body that still (rarely) races and errors with `UND_ERR_SOCKET`/"other side closed" is also turned back into a normal end as a backstop, for every `responseType` including `'stream'` - this backstop alone, without the eager drain, is not a reliable fix for `'stream'` or a response with no `Content-Length` (chunked, unknown length), both of which remain exposed to the underlying bug - see [`plan/reports/undici-slow-consumer.md`](/plan/reports/undici-slow-consumer.md). |
