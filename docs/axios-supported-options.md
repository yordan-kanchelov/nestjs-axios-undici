...
| `data`: object → JSON, string, `URLSearchParams`, `Buffer`/typed arrays, streams, `FormData` (global or the `form-data` package) | ✅ | Same `Content-Type` defaults as axios. A bare `number`/`boolean` body (not axios' documented shapes) is forwarded stringified instead of rejected - see [Errors](#errors). |
| A `Content-Length` header that doesn't match the body's actual byte length | ⚠️ | Rejected as an `AxiosError` with `error.code === 'UND_ERR_REQ_CONTENT_LENGTH_MISMATCH'` (undici's own code, kept as-is - not remapped to an axios code; a request-smuggling protection). axios' own Node `http` transport trusts a caller-supplied `Content-Length` and sends it as given. Leave `Content-Length` unset (the default) to have it computed correctly. |
...
| `sensitiveHeaders` (axios ≥1.x, an array of extra header names) | ✅ | Extra header names (case-insensitive) dropped alongside `Authorization`/`Cookie`/`Proxy-Authorization`. Unlike that built-in 3 (dropped only on a protocol downgrade or a cross-host redirect that isn't to a subdomain), a header named here is dropped on *any* change of origin - including a subdomain redirect or an http→https upgrade - matching axios' own, stricter rule for this option (`isSameOriginRedirect` in `lib/adapters/http.js`). Must be an array of strings, or rejects with `ERR_BAD_OPTION_VALUE`, "sensitiveHeaders must be an array of strings" - same as axios; validated up front, whenever redirects are actually followed (`maxRedirects` isn't `0`). Also settable on `axiosRef.defaults`/`axiosRef.create({ sensitiveHeaders })` and at module level (`register({ sensitiveHeaders })`), same request > `axiosRef.defaults` > module precedence as every other option - a per-request value wins outright (not merged). |
...
| `decompress` | ✅ | gzip/compress/x-compress/br/deflate/zstd are decompressed when `Content-Encoding` is set (`responseType: 'stream'` included - the decompression stream is destroyed, alongside the raw response body/socket behind it, if a consumer stops reading early). `compress`/`x-compress` decode exactly like `gzip` (axios itself only aliases the name onto its gzip decoder; it doesn't implement the old LZW `compress` scheme either). `decompress: false` returns the raw compressed body, as in axios. `zstd` decoding is feature-detected (`zlib.createZstdDecompress`, present on every Node.js version this package supports - added in Node 22.15.0/23.8.0) the same way axios does; on a hypothetical Node build that lacks it, a `Content-Encoding: zstd` response is returned as the raw compressed bytes, matching axios' own fallback. Every decoder uses axios' own flush-tolerant zlib options (`finishFlush: Z_SYNC_FLUSH` etc.), so an empty or truncated-but-structurally-valid compressed body resolves (with `''`, or with whatever partial bytes could be decoded) instead of throwing - only data that fails the format check entirely (the wrong bytes altogether) rejects; see the `Content-Encoding` row under [Response](#response) for that error's shape. |
...
**`axiosRef.create(config)`** merges `config` onto the parent's `defaults` the same way (`headers` per-bucket, everything else above overridden outright when `config` sets it) - matching axios' own `axios.create(config)`, which merges `config` onto `this.defaults` via `mergeConfig`. Every field in the list above is honoured this way, including `auth`/`maxContentLength`/`maxBodyLength`/`timeoutErrorMessage`/`decompress`/`socketPath`/`allowAbsoluteUrls`/`beforeRedirect` (review fix - these 8 used to be silently dropped: `create({ auth })` sent no credentials, and `create({ maxContentLength })` enforced nothing, because the request-normalisation code only ever read them off module options, never off `defaults`).

A few axios options are **not** part of this merge surface, by design:
- **`httpAgent` / `httpsAgent` / `proxy` / `httpVersion` / `cookieJar`** are resolved into this `HttpService`'s dispatcher once, at construction (see [Configuring undici directly](#configuring-undici-directly)) - `axiosRef.create()` explicitly shares that dispatcher/transport rather than building its own, so these can't be re-resolved per `create()`/`defaults` without rebuilding a dispatcher (a deliberate performance choice, not an oversight). `socketPath` is the one exception: unlike those, it's already resolved dynamically per request (a small cache keyed by path), so it *is* part of `axiosRef.defaults`/`create()` like every other option above.
- **`signal`** isn't mergeable into a shared default, in this library or in axios itself: one `AbortSignal` firing would abort every request still using it, so it only ever makes sense per request.
- **`xsrfCookieName` / `xsrfHeaderName`** do nothing at any level (see the module-level table below) - nothing to merge.
- **`insecureHTTPParser` / `lookup` / `family`** aren't implemented by this library at all, at any level (module, `defaults`, or per-request) - not axios options this library has ever mapped to undici, so there's nothing for `create()`/`defaults` to drop.
...
| `Content-Encoding: gzip \| compress \| x-compress \| br \| deflate \| zstd` | ✅ | Decompressed automatically (`compress`/`x-compress` decode exactly like `gzip`, matching axios); `decompress: false` opts out. The header itself is then deleted from `response.headers`, matching axios exactly (only when something was actually decoded - never with `decompress: false`, and never for an encoding this library doesn't recognize at all) - **breaking change from 0.6**: a caller reading `response.headers['content-encoding']` after a successful decode now sees `undefined`, where it used to still show the original (now-inaccurate) value. `content-length` is left untouched either way (axios doesn't touch it on decode either - it still reflects the *compressed* size). A body that fails to decode entirely (the wrong format, not just truncated) rejects as a real `AxiosError` (`isAxiosError`/`.config`/`.request`/`.code` all set, `code` falling back to the raw zlib code such as `Z_DATA_ERROR`) for both a buffered response and `responseType: 'stream'` (the stream's own `'error'` event carries it) - matching axios' buffered-path shape exactly (`AxiosError.from(err, null, config, lastRequest, response)`), and extended here to the stream path too, which axios itself leaves unwrapped. An empty or merely truncated (but structurally valid) compressed body never throws at all, resolving with `''` or with whatever could be decoded from the partial bytes - axios' own flush-tolerant zlib options (`finishFlush: Z_SYNC_FLUSH` etc.), matched here. |
