...
| `sensitiveHeaders` (axios ≥1.x, an array of extra header names) | ❌ | Not implemented (plan.md: "fix: redirect sensitiveHeaders option"). `Authorization`/`Cookie`/`Proxy-Authorization` are always dropped across a protocol downgrade or host change (a fixed set, matching axios' own default); axios lets `sensitiveHeaders` extend that set with your own header names, which this library ignores if set. |
...
| `decompress` | ⚠️ | gzip/br/deflate are decompressed when `Content-Encoding` is set. `decompress: false` returns the raw compressed body, as in axios. **`zstd` isn't decompressed at all** (plan.md: "fix: zstd decompression") - axios feature-detects `zlib.createZstdDecompress` and decodes it when the Node.js runtime exposes it (it does on every Node.js version this package supports); a `Content-Encoding: zstd` response here is returned as the raw compressed bytes regardless of `decompress`. |
...
| `parseReviver` | ❌ | Not implemented (plan.md: "fix: JSON reviver (parseReviver)"). axios passes it as the second argument to `JSON.parse(data, config.parseReviver)` when default JSON parsing applies; here, JSON responses are always parsed with a plain `JSON.parse(data)`, ignoring `parseReviver` if set. |
...
| `Content-Encoding: zstd` | ❌ | Not decompressed - returned as the raw compressed `Buffer` regardless of `decompress` (plan.md: "fix: zstd decompression"). axios decodes it when the Node.js runtime exposes `zlib.createZstdDecompress`. |
