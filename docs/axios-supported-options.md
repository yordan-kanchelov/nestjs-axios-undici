...
| `axiosRef.defaults.baseURL` / `.timeout` / `.maxRedirects` / `.params` / `.paramsSerializer` / `.validateStatus` / `.responseType` / `.transformRequest` / `.transformResponse` / `.adapter` / `.withCredentials` / `.onUploadProgress` / `.onDownloadProgress` / `.maxRate` / `.formSerializer` / `.parseReviver` / `.sensitiveHeaders` | ✅ | Honoured at request time, including on a plain request with no axiosRef interceptors. |
